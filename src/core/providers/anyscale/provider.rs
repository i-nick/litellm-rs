...
impl AnyscaleProvider {
    pub fn new(config: AnyscaleConfig) -> Result<Self, ProviderError> {
        config.validate().map_err(|e| {
            ProviderError::configuration("anyscale", e)
        })?;
        ...
        let http_client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|e| {
                ProviderError::initialization("anyscale", format!("Failed to create HTTP client: {}", e))
            })?;
...
impl LLMProvider for AnyscaleProvider {
    ...
    async fn chat_completion_stream(
        ...
        Err(ProviderError::not_implemented("anyscale", "Streaming not yet implemented"))
    }
...
mod tests {
    ...
    fn test_provider_models() {
        ...
        assert!(models.iter().any(|m| m.id == "meta-llama/Llama-2-70b-chat-hf"));
        assert!(models.iter().any(|m| m.id == "mistralai/Mistral-7B-Instruct-v0.1"));
        assert!(models.iter().any(|m| m.id == "codellama/CodeLlama-34b-Instruct-hf"));
    }
}
