...
pub struct CometApiConfig {
    #[serde(flatten)]
    pub base: BaseConfig,
}

impl Default for CometApiConfig {
    fn default() -> Self {
        Self {
            base: BaseConfig {
                api_key: None,
...
impl CometApiConfig {
    ...
    pub fn from_env() -> Result<Self, String> {
        let api_key = std::env::var("COMET_API_KEY")
            .map_err(|_| "COMET_API_KEY environment variable is required")?;
        Ok(Self::new(api_key))
    }
...
impl ProviderConfig for CometApiConfig {
    fn validate(&self) -> Result<(), String> {
        self.base.validate("cometapi")?;
        Ok(())
    }
