...
impl DataRobotProvider {
    ...
    pub fn new(config: DataRobotConfig) -> Result<Self, ProviderError> {
        config
            .validate()
            .map_err(|e| ProviderError::configuration("datarobot", e))?;

        let pool_manager = Arc::new(
            GlobalPoolManager::new()
                .map_err(|e| ProviderError::configuration("datarobot", e.to_string()))?,
        );
        let supported_models = DataRobotClient::supported_models();
...
#[async_trait]
impl LLMProvider for DataRobotProvider {
    type Config = DataRobotConfig;
    type Error = ProviderError;
    type ErrorMapper = DataRobotErrorMapper;
    ...
    fn get_supported_openai_params(&self, _model: &str) -> &'static [&'static str] {
        DataRobotClient::supported_openai_params()
    }
    ...
    async fn transform_request(
        ...
        Ok(DataRobotClient::transform_chat_request(request))
    }
    ...
    async fn transform_response(
        ...
        let response: ChatResponse = serde_json::from_slice(raw_response)
            .map_err(|e| ProviderError::response_parsing("datarobot", e.to_string()))?;
        Ok(response)
    }

    fn get_error_mapper(&self) -> Self::ErrorMapper {
        DataRobotErrorMapper
    }
    ...
    async fn chat_completion(
        ...
        let body = DataRobotClient::transform_chat_request(request.clone());
        ...
        let response_bytes = response
            .bytes()
            .await
            .map_err(|e| ProviderError::network("datarobot", e.to_string()))?;
    ...
    ) -> Result<Pin<Box<dyn Stream<Item = Result<ChatChunk, Self::Error>> + Send>>, Self::Error>
    {
        ...
        let mut body = DataRobotClient::transform_chat_request(request.clone());
        body["stream"] = serde_json::Value::Bool(true);

        let api_key = self
            .config
            .base
            .get_effective_api_key("datarobot")
            .ok_or_else(|| ProviderError::authentication("datarobot", "API key is required"))?;
        ...
        Ok(Box::pin(super::streaming::create_datarobot_stream(stream)))
    }
