...
impl AlephAlphaProvider {
    pub fn new(config: AlephAlphaConfig) -> Result<Self, ProviderError> {
        config.validate().map_err(|e| {
            ProviderError::configuration("aleph_alpha", e)
        })?;
        ...
        let http_client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|e| {
                ProviderError::initialization("aleph_alpha", format!("Failed to create HTTP client: {}", e))
            })?;
...
impl LLMProvider for AlephAlphaProvider {
    ...
    async fn chat_completion_stream(
        ...
        Err(ProviderError::not_implemented("aleph_alpha", "Streaming not yet implemented"))
    }
